...
export const useSeatLayout = () => {
  const { postData, data, message, loading, status, error, isError, setData } = usePostData();
...
export const usePostCanceledOrder = () => {
  const { postData, data, message, loading, status, error, isError } = usePostData();

  return {
    fetchCanceledOrder: ({ header, option, queryParams, body }: IPostData) => {
      const apiOptions = {
      ...
      return postData(apiOptions);
    },
    canceledOrder: data,
    canceledOrderMessage: message,
    canceledOrderLoading: loading,
    canceledOrderStatus: status,
    canceledOrderError: error || data?.message,
    canceledOrderIsError: isError || data?.status == '1',
  };
}
